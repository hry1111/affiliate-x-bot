 ...
 */
export function composePostVariants({ product, item, disclosureText = '【PR】', socialStrategy = null }) {
  if (socialStrategy?.post_type === 'REJECT') return [];
  ...
  const lead = socialStrategy?.hook ? `${socialStrategy.hook}\n\n` : '';
  ...
  if (product.postMode === 'owned') {
    ...
    return [
      ...
      },
      {
        id: 'owned-caution',
        label: '失敗回避',
        text: `${lead}${problem}なら、${productName}は候補に入れてよかった。\n\n${reason}\n\nただ、${caution}\n\n${footer}`,
      },
    ];
  }
    ...
    return [
      {
        id: 'sale-check',
        label: '今見る',
        text: `${lead}これ気になってた人は、今なら見ておいてよさそう。\n\n${reason}\n\n${caution}\n\n${footer}`,
      },
      {
        id: 'sale-target',
        label: '対象者向け',
        text: `${lead}${target}なら、${productName}が買いやすい価格になってる。\n\n${reason}\n\n${caution}\n\n${footer}`,
      },
      {
        id: 'sale-caution',
        label: '確認してから',
        text: `${lead}安いからといって、誰にでも合うわけではない。\n\n${problem}なら、これは候補。\n${caution}\n\n${footer}`,
      },
    ];
  }
  ...
  return [
    {
      id: 'research-natural',
      label: '自然な紹介',
      text: `${lead}${problem}で迷ってる人は、これ候補に入れてよさそう。\n\n${reason}\n\n${caution}\n\n${footer}`,
    },
    {
      id: 'research-target',
      label: '対象者向け',
    ...
    },
    {
      id: 'research-source',
      label: '比較根拠あり',
      text: `${lead}${problem}なら、比較記事を見た上でこれは候補。\n\n${sourceNames}で紹介されていて、${reason}\n\n${caution}\n\n${footer}`,
    },
  ];
}
