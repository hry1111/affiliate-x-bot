...
`config/seasonal-topics.json`では、季節ごとに探すテーマと楽天ランキングの絞り込み条件を設定します。現在は、夏の暮らし、防災、日用品・家事、キッチン・調理、収納・インテリア、美容・身だしなみ、スマホ・PC周辺、車用品、ペット用品、子育て・ベビーの10ジャンルを登録しています。
...
実体験は任意です。`owned`以外の使用感は生成せず、未使用の商品は一次情報、確認日時付きのランキング情報、比較軸、購入導線、参考記事の非転載確認、`【PR】`がそろうまでSNS・楽天ROOM下書きは生成しません。
